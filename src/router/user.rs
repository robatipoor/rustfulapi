...
pub fn add_routers(router: axum::Router<AppState>) -> axum::Router<AppState> {
  router
    ...
    .route("/api/v1/users/login2fa", post(user::login2fa))
    .route("/api/v1/users/refresh-token", get(user::refresh_token))
    .route("/api/v1/users/token-info", post(user::refresh_token))
